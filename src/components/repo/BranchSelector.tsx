...
export const BranchSelector = ({
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={handleOpen}
        className="flex items-center gap-2 px-3 py-1.5 text-sm bg-zinc-50 border border-zinc-200 rounded-md hover:bg-zinc-100 transition-colors text-zinc-700 font-medium"
      >
        <Icon name="solar:branch-linear" size={16} />
        {currentBranch}
        <Icon name="solar:alt-arrow-down-linear" size={12} className="ml-1 text-zinc-400" />
      </button>

      {isOpen ? (
        <div className="absolute top-full left-0 mt-1 w-48 bg-white border border-zinc-200 rounded-md shadow-lg z-50">
          <div className="p-2 border-b border-zinc-100">
            <span className="text-xs font-medium text-zinc-500">Switch branches</span>
          </div>
            ...
            {isLoading ? (
              ...
              branches.map((branch) => (
                <button
                  ...
                  className={cn(
                    'w-full text-left px-3 py-1.5 text-sm hover:bg-zinc-50 transition-colors flex items-center gap-2',
                    branch === currentBranch && 'text-zinc-900 font-medium'
                  )}
                >
